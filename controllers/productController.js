...
export const razorderid = async(req,res)=>{
...
export const payment=async(req,res)=>{
  try {
    const { orderId, paymentId, signature,buyerid} = req.body;
   
    console.log({ orderId, paymentId, signature,buyerid });
 ...
 // var crypto = require("crypto");
  ...
  }else
  {
    ...
    res.status(500).send('Failed to handle payment success');
  }
}
...
instance.orders.create(options, function(err, order) {
  if(err)
  {
    ...
    res.send({message:'OrderId created',data:order});
  }
  console.log(order);
});
  } catch (error) {
